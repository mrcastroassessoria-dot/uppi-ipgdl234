  ...
  return (
    <header className="h-16 border-b border-border bg-card/80 backdrop-blur-xl flex items-center justify-between px-6 shrink-0">
      ...
      <div className="flex items-center gap-4">
        ...
        {/* Notifications */}
        <button type="button" className="relative w-9 h-9 rounded-xl bg-secondary flex items-center justify-center hover:bg-secondary/80 transition-colors">
        ...
        <div className="flex items-center gap-2.5">
          ...
          {profile && (
            <span className="hidden lg:block text-[13px] font-semibold text-foreground">{profile.full_name}</span>
          )}
        </div>
      </div>
    </header>
