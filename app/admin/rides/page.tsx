...
import {
  ...
  Navigation, XCircle, CheckCircle, Loader2, Radio,
} from 'lucide-react'
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
            ...
            {filters.map((f) => (
              <button
                ...
                className={cn(
                  ...
                  statusFilter === f.key ? 'bg-blue-600 text-white' : 'bg-secondary text-muted-foreground hover:text-foreground'
                )}
              >
                {f.label}
              </button>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {filtered.map((ride) => {
            ...
            return (
              <Card key={ride.id} className="border-border/50 overflow-hidden">
                <button
                  type="button"
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <Badge className={cn('text-[10px] font-bold', sc.color)}>{sc.label}</Badge>
                        ...
                        <div className="flex items-center gap-2 text-[13px]">
                          ...
                          <span className="text-foreground truncate">{ride.pickup_address || 'N/A'}</span>
                        </div>
                        <div className="flex items-center gap-2 text-[13px] mt-0.5">
                          <MapPin className="w-3 h-3 text-red-500 shrink-0" />
                          <span className="text-foreground truncate">{ride.dropoff_address || 'N/A'}</span>
                        </div>

                        <div className="flex items-center gap-3 mt-2 text-[12px] text-muted-foreground">
                          {ride.passenger && (
                            <span className="flex items-center gap-1">
                              <User className="w-3 h-3" /> {ride.passenger.full_name}
                            </span>
                          )}
                          {ride.driver && (
                            <span className="flex items-center gap-1">
                              <Car className="w-3 h-3" /> {ride.driver.full_name}
                            </span>
                          )}
                          {ride.final_price > 0 && (
                ...
                {/* Expanded Details */}
                {isOpen && (
                  <div className="border-t border-border/50 bg-secondary/30 p-4 space-y-4">
                    {/* Route Map Preview */}
                    {ride.pickup_lat && ride.dropoff_lat && (
                      <div className="h-[200px] rounded-xl overflow-hidden border border-border/50">
                        <img
                          src={`https://maps.googleapis.com/maps/api/staticmap?size=600x200&markers=color:green|${ride.pickup_lat},${ride.pickup_lng}&markers=color:red|${ride.dropoff_lat},${ride.dropoff_lng}&path=color:0x3b82f6ff|weight:4|${ride.pickup_lat},${ride.pickup_lng}|${ride.dropoff_lat},${ride.dropoff_lng}&style=feature:all|element:geometry|color:0x1a1a2e&style=feature:water|color:0x0d1b2a&key=${process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY || ''}`}
                          alt="Rota"
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      <div className="bg-card rounded-xl p-3 border border-border/50">
                        <p className="text-[11px] text-muted-foreground mb-0.5">Distancia</p>
                        <p className="text-[15px] font-bold text-foreground">{ride.distance_km ? `${Number(ride.distance_km).toFixed(1)} km` : 'N/A'}</p>
                      </div>
                      <div className="bg-card rounded-xl p-3 border border-border/50">
                        <p className="text-[11px] text-muted-foreground mb-0.5">Duracao Est.</p>
                        <p className="text-[15px] font-bold text-foreground">{ride.estimated_duration_minutes ? `${ride.estimated_duration_minutes} min` : 'N/A'}</p>
                      </div>
                      <div className="bg-card rounded-xl p-3 border border-border/50">
                        <p className="text-[11px] text-muted-foreground mb-0.5">Oferta Pass.</p>
                        <p className="text-[15px] font-bold text-foreground">{ride.passenger_price_offer ? `R$ ${Number(ride.passenger_price_offer).toFixed(2)}` : 'N/A'}</p>
                      </div>
                      <div className="bg-card rounded-xl p-3 border border-border/50">
                        <p className="text-[11px] text-muted-foreground mb-0.5">Valor Final</p>
                        <p className="text-[15px] font-bold text-emerald-500">{ride.final_price ? `R$ ${Number(ride.final_price).toFixed(2)}` : 'N/A'}</p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div className="bg-card rounded-xl p-3 border border-border/50">
                        <p className="text-[11px] text-muted-foreground mb-1.5 flex items-center gap-1"><User className="w-3 h-3" /> Passageiro</p>
                        <p className="text-[14px] font-semibold text-foreground">{ride.passenger?.full_name || 'N/A'}</p>
                        <p className="text-[12px] text-muted-foreground">{ride.passenger?.phone || ''}</p>
                      </div>
                      <div className="bg-card rounded-xl p-3 border border-border/50">
                        <p className="text-[11px] text-muted-foreground mb-1.5 flex items-center gap-1"><Car className="w-3 h-3" /> Motorista</p>
                        <p className="text-[14px] font-semibold text-foreground">{ride.driver?.full_name || 'Sem motorista'}</p>
                        <p className="text-[12px] text-muted-foreground">{ride.driver?.phone || ''}</p>
                      </div>
                    </div>
                    ...
                    {ride.cancellation_reason && (
                      <div className="bg-red-500/10 rounded-xl p-3 text-[12px] text-red-400">
                    ...
                    {ride.status !== 'completed' && ride.status !== 'cancelled' && (
                      <button
                        ...
                        className="w-full h-10 rounded-xl bg-red-500/10 text-red-500 text-[13px] font-bold hover:bg-red-500/20 transition-colors flex items-center justify-center gap-2"
                      >
                        <XCircle className="w-4 h-4" /> Cancelar Corrida (Admin)
                      </button>
                    )}

                    <p className="text-[10px] text-muted-foreground font-mono">ID: {ride.id}</p>
                  </div>
                )}
              </Card>
            )
