...
import {
  Car, Radio, Users, MapPin, Navigation, Clock, Eye, Loader2,
} from 'lucide-react'
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-hidden flex">
        ...
        <div className="hidden lg:flex w-[340px] flex-col border-l border-border bg-background overflow-hidden">
          <div className="p-4 border-b border-border">
            <h2 className="text-[15px] font-bold text-foreground flex items-center gap-2">
              <Radio className="w-4 h-4 text-blue-500" />
              Corridas Ativas
              <Badge className="bg-blue-500/15 text-blue-500 text-[10px] ml-auto">{rides.length}</Badge>
            </h2>
          </div>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    selectedRide === r.id ? 'bg-blue-600/10 ring-1 ring-blue-500/30' : 'hover:bg-secondary/70'
                  )}
                >
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-1.5 text-[12px]">
                      ...
                      <span className="text-foreground truncate">{r.pickup_address || 'N/A'}</span>
                    </div>
                    <div className="flex items-center gap-1.5 text-[12px]">
                      <div className="w-2 h-2 rounded-full bg-red-500 shrink-0" />
                      <span className="text-foreground truncate">{r.dropoff_address || 'N/A'}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 mt-2 text-[11px] text-muted-foreground">
                    {r.passenger && <span className="flex items-center gap-1"><Users className="w-3 h-3" /> {r.passenger.full_name}</span>}
                    {r.driver && <span className="flex items-center gap-1"><Car className="w-3 h-3" /> {r.driver.full_name}</span>}
                    {r.final_price > 0 && <span className="ml-auto font-semibold text-foreground">R$ {Number(r.final_price).toFixed(2)}</span>}
                  </div>
                </button>
              )
            })}
          ...
          <div className="border-t border-border">
            <div className="p-3 border-b border-border/50">
              <h3 className="text-[13px] font-bold text-foreground flex items-center gap-2">
                ...
                Motoristas Online
                <span className="text-[10px] text-muted-foreground ml-auto">{onlineDrivers.length}</span>
              </h3>
            </div>
            <div className="max-h-[200px] overflow-y-auto p-2 space-y-1">
              {onlineDrivers.slice(0, 10).map((d) => (
                <button
                  ...
                  className="w-full flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-secondary/70 transition-colors"
                >
                  <div className="w-2 h-2 rounded-full bg-emerald-500 shrink-0" />
                  <span className="text-[12px] text-foreground font-medium truncate flex-1 text-left">
                    {d.profile?.full_name || 'Motorista'}
                  </span>
                  <span className="text-[10px] text-muted-foreground">{d.vehicle_plate}</span>
                </button>
              ))}
              {onlineDrivers.length === 0 && (
                <p className="text-[12px] text-muted-foreground text-center py-4">Nenhum motorista online</p>
              )}
            </div>
          </div>
        </div>
