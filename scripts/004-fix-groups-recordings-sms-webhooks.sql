-- ============================================================
-- UPPI - Fase 4 (fixed): Group rides + recordings + SMS + webhooks
-- Tables first, then policies referencing other tables
-- ============================================================

-- ============================================================
-- Group Rides - Tables first
-- ============================================================
...
-- Now add policies that cross-reference both tables
...
CREATE POLICY "participants_select" ON public.group_ride_participants
  FOR SELECT USING (
    EXISTS (
      ...
      WHERE gr.id = group_ride_participants.group_ride_id
        AND (gr.creator_id = auth.uid() OR auth.uid() = group_ride_participants.user_id)
    )
  );
...
CREATE POLICY "sms_templates_select" ON public.sms_templates FOR SELECT USING (is_active = true);
